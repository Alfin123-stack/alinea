 ...
 *
 * Adapted for Alinea: no drop shadow (the site's monochrome system doesn't
 * use them), cards carry the site's own radius token, and a card can
 * navigate — a book cover in this ring is meant to be opened, not just
 * looked at.
 * ─────────────────────────────────────────────────────────────── */
...
export type HaloReelItem = {
  ...
  title?: string;
  subtitle?: string;
  /** Where the card goes when clicked/tapped. Omit for a non-interactive ring. */
  href?: string;
};
...
export function HaloReel({
  ...
  return (
    <div
      ...
      aria-label={props["aria-label"] ?? "Book carousel"}
      tabIndex={0}
      onKeyDown={onKeyDown}
      ...
      className={cn(
        "relative h-[420px] w-full touch-pan-y select-none overflow-hidden outline-none md:h-[520px]",
        draggable && "cursor-grab active:cursor-grabbing",
        "focus-visible:ring-2 focus-visible:ring-ink focus-visible:ring-inset",
        className,
      )}
      style={style}
      {...props}
...
function WheelCard({
  ...
  const face = item.src ? (
    ...
    <div
      className="flex h-full w-full flex-col items-center justify-center gap-1 bg-paper p-3 text-center text-ink"
      style={{
    ...
    >
      {item.title ? (
        <span className="font-serif text-xl leading-none">{item.title}</span>
      ) : null}
      {item.subtitle ? (
        <span className="type-caption uppercase tracking-[0.2em] opacity-70">
          {item.subtitle}
        </span>
      ) : null}
    </div>
  ...
  return (
    <motion.div
      ...
      className="absolute overflow-hidden rounded-2xl ring-1 ring-ink/10"
    >
