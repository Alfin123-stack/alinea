...
export type SqueezeSlide = {
  ...
  background?: string;
  /** Sits top-left of every panel, slat or open: a curator credit, a wordmark. */
  overlay?: ReactNode;
  /** Sits bottom of a *collapsed* panel only — hidden on the one that's open. */
  caption?: ReactNode;
...
export type SqueezeCarouselProps = {
  ...
  gap?: Size;
  /** Corner rounding on a panel. Default `16` — Alinea's standard radius. */
  radius?: Size;
  ...
  controls?: boolean;
  /** Fill behind the button, the arrows and the focus ring. Defaults to `--color-ink`. */
  accent?: string;
  /** What sits on top of that fill — the label and the arrow heads. Defaults to `--color-paper`. */
...
export function SqueezeCarousel({
  ...
  interval = 6000,
  controls = true,
  accent = "var(--sq-accent, var(--color-ink))",
  accentForeground = "var(--sq-accent-foreground, var(--color-paper))",
  label = "Featured",
  panelClassName,
  className,
  style,
  ...
  // Initial batch keyed 0..visible-1 without touching the `seed` ref during
  // render (React 19's stricter refs rule flags any ref read inside a
  // useState lazy initializer, even though it only ever runs once, before
  // mount). `seed` picks up from `visible` so later keys never collide.
  const [cards, setCards] = useState<Card[]>(() =>
    Array.from({ length: visible }, (_, p) => ({ key: p, slide: wrap(defaultIndex + p) })),
  );
  ...
  return (
    ...
    >
          ...
          {cards.map((card, place) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "relative isolate h-full shrink-0 cursor-pointer overflow-hidden bg-linen p-0",
                  "outline-none focus-visible:ring-2 focus-visible:ring-offset-2",
                  "focus-visible:ring-[var(--sq-fill)] focus-visible:ring-offset-linen",
                  panelClassName,
                )}
                style={{
                  width: widthOf(col),
              ...
              >
                ...
                {slide.overlay && (
                  <span className="pointer-events-none absolute left-3 top-3 type-caption text-paper/80">
                    {slide.overlay}
                  </span>
                )}
                ...
                {slide.caption && (
                  <span
                    aria-hidden="true"
                    className="pointer-events-none absolute inset-x-3 bottom-3 truncate type-caption text-paper/85"
                    style={{
                      opacity: front ? 0 : 1,
      ...
      <div id={`${ids}-panel`} role="tabpanel" aria-live="polite" className="mt-6 grid @container">
        {slides.map((slide, i) => {
          const shown = i === open;
          ...
          return (
            <div
              key={slide.id ?? i}
              aria-hidden={!shown}
              className="col-start-1 row-start-1 flex flex-col items-start justify-between gap-4 @sm:flex-row @sm:items-center @sm:gap-10"
              style={{
                opacity: shown ? 1 : 0,
            ...
            >
              <div>
                <p className="type-subheading text-ink">{slide.title}</p>
                {slide.description && (
                  <p className="type-caption mt-1 text-stone">{slide.description}</p>
                )}
              </div>
...
function Action({ slide, shown }: { slide: SqueezeSlide; shown: boolean }) {
  ...
  const dress = cn(
    ...
    "bg-[var(--sq-fill)] px-5 py-2.5 type-caption text-[var(--sq-on-fill)]",
